...
# 순환신경망만들기
# rnn1 : 단어임베딩 사용
model_rnn1 = keras.Sequential()
model_rnn1.add(keras.layers.Embedding(500, 16, input_shape=(100,)))
model_rnn1.add(keras.layers.SimpleRNN(8))
model_rnn1.add(keras.layers.Dense(1, activation='sigmoid')) #긍정, 부정 이진분류
model_rnn1.summary()
...
save_point_rnn1 = keras.callbacks.ModelCheckpoint('best-rnn1-model.keras', save_best_only=True)
early_stop_point = keras.callbacks.EarlyStopping(patience=3, restore_best_weights=True)
his_rnn1 = model_rnn1.fit(tr_padding, train_target, epochs=100, batch_size=64, validation_data=(val_padding, val_target), callbacks=[save_point_rnn1, early_stop_point])

plt.plot(his_rnn1.history['loss'])
plt.plot(his_rnn1.history['val_loss'])
plt.xlabel('epoch')
plt.ylabel('loss')
